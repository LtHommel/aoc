module Aoc2022.Day5.Part1 where
...
move (n, from, to) stacks = move (n -1, from, to) (replace from fromStackAfter $ replace to (push popped toStack) stacks)
  where
    fromStack = stacks !! (from -1)
    toStack = stacks !! (to -1)
    popped = head fromStack
    fromStackAfter = tail fromStack
